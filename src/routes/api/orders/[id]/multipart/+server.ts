...
export async function PUT(event: RequestEvent) {
  try {
    ...
    if (userRole !== 'admin' && userRole !== 'staff') {
      return json({ message: 'Forbidden: hanya admin dan staff yang dapat mengupdate order' }, { status: 403 });
    }
    ...
    const status = formData.get('status') as 'pending' | 'processing' | 'finished' | 'canceled' | undefined;
    const shippingMethod = formData.get('shippingMethod') as 'pickup' | 'delivery' | undefined;
    const paymentMethod = formData.get('paymentMethod') as 'transfer' | 'qris' | 'cash' | undefined;
    const paymentStatus = formData.get('paymentStatus') as 'pending' | 'confirmed' | 'failed' | 'refunded' | undefined;
    const totalAmount = formData.get('totalAmount') ? Number(formData.get('totalAmount')) : undefined;
    const notes = formData.get('notes') as string | undefined;
    const createdById = formData.get('createdById') ? Number(formData.get('createdById')) : undefined;
    ...
    if (paymentProofFile) {
      ...
      if (targetPaymentMethod === 'transfer' || targetPaymentMethod === 'qris') {
        ...
        return json({ message: 'Payment proof hanya bisa diupload untuk metode transfer atau qris' }, { status: 400 });
      }
    }
