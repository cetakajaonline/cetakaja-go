...
export async function PUT(event: RequestEvent) {
  try {
    ...
    if (!notificationId || !status) {
      return json({ message: 'ID notifikasi dan status wajib disertakan' }, { status: 400 });
    }
    
    if (!['sent', 'failed'].includes(status)) {
      return json({ message: 'Status tidak valid. Gunakan "sent" atau "failed"' }, { status: 400 });
    }
    ...
    return json({ message: 'Gagal memperbarui status notifikasi' }, { status: 500 });
  }
}