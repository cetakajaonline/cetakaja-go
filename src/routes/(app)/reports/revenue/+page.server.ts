...
export const load: PageServerLoad = async ({ url }) => {
  try {
    ...
    if (startDateParam && endDateParam) {
      ...
      // Get revenue report data for date range using the service
      reportData = await getRevenueReportForDateRange(startDate, endDate);
    } else if (dateParam) {
      // Single date query
      const parsedDate = z
        .string()
      ...
      // Get revenue report data for single date using the service
      reportData = await getRevenueReport(targetDate);
    } else {
    ...
    throw error(500, "Failed to fetch revenue report");
  }
};