<script lang="ts">
  ...
  import RevenueReportChart from "$lib/components/reports/RevenueReportChart.svelte";
  import RevenueReportSummary from "$lib/components/reports/RevenueReportSummary.svelte";
  import RevenueReportDetails from "$lib/components/reports/RevenueReportDetails.svelte";
  import type { RevenueReportData } from "$lib/types";
  import { onMount } from "svelte";
  import { getRevenueReportForDateRange } from "$lib/services/reportClient";

  export let data: { reportData: RevenueReportData };
  ...
  async function refreshReport(startDate: Date, endDate: Date) {
    ...
    window.location.href = `/reports/revenue?startDate=${startFormattedDate}&endDate=${endFormattedDate}`;
  }
  ...
  async function exportToPDF() {
    try {
      const { exportRevenueReportToPDF } = await import(
  ...
  async function exportToExcel() {
    try {
      const { exportRevenueReportToExcel } = await import(
...
<DefaultLayout title="Revenue Report">
  ...
  <PageHeader title="Laporan Pendapatan" icon="💰" />
  ...
  <div class="p-6">
    ...
    <div class="alert alert-info mb-6">
      <div>
        <span
          >Laporan pendapatan: <strong>{formattedStartDate}</strong> -
    ...
    <!-- Report Summary -->
    <div
      class="card bg-gradient-to-r from-cyan-500 to-teal-600 text-white shadow-xl mb-8"
    >
      <div class="card-body">
        <RevenueReportSummary {reportData} />
      </div>
    </div>
    ...
    <div class="card bg-base-100 shadow-xl">
      <div class="card-body">
        <RevenueReportDetails {reportData} />
      </div>
    </div>
  </div>
</DefaultLayout>
