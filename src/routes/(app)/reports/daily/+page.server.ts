...
import { getDailyReport as getDailyReportService } from '$lib/server/reportService';
import { z } from 'zod';

/**
 * Loads daily report data for the specified date.
 * If no date is provided in the URL, defaults to the current day.
 * 
 * @param url - The URL object containing query parameters
 * @returns Object containing daily report data
 */
export const load: PageServerLoad = async ({ url }) => {
  try {
    ...
    if (dateParam) {
      ...
      targetDate = new Date(today.getFullYear(), today.getMonth(), today.getDate(), 0, 0, 0, 0);
    }

    // Get daily report data using the service
    const reportData = await getDailyReportService(targetDate);
    ...
    throw error(500, 'Failed to fetch daily report');
  }
};