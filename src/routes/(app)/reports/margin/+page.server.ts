...
import {
  getMarginReport,
  getMarginReportForDateRange,
} from "$lib/server/reportService";
import { z } from "zod";

/**
 * Loads margin report data for the specified date or date range.
...
export const load: PageServerLoad = async ({ url }) => {
  try {
    ...
    if (startDateParam && endDateParam) {
      ...
      // Get margin report data for date range using the service
      reportData = await getMarginReportForDateRange(startDate, endDate);
    } else if (dateParam) {
      // Single date query
      const parsedDate = z
        .string()
      ...
      // Get margin report data for single date using the service
      reportData = await getMarginReport(targetDate);
    } else {
