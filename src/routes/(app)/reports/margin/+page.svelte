<script lang="ts">
  ...
  import MarginReportChart from "$lib/components/reports/MarginReportChart.svelte";
  import MarginReportSummary from "$lib/components/reports/MarginReportSummary.svelte";
  import MarginReportDetails from "$lib/components/reports/MarginReportDetails.svelte";
  import type { MarginReportData } from "$lib/types";
  import { onMount } from "svelte";
  import { getMarginReportForDateRange } from "$lib/services/reportClient";

  export let data: { reportData: MarginReportData };
  ...
  async function refreshReport(startDate: Date, endDate: Date) {
    ...
    window.location.href = `/reports/margin?startDate=${startFormattedDate}&endDate=${endFormattedDate}`;
  }
  ...
  async function exportToPDF() {
    try {
      const { exportMarginReportToPDF } = await import(
        "$lib/utils/pdfExporter"
      );
      await exportMarginReportToPDF(
        reportData,
  ...
  async function exportToExcel() {
    try {
      const { exportMarginReportToExcel } = await import(
        "$lib/utils/excelExporter"
      );
      await exportMarginReportToExcel(
        reportData,
        reportData.startDate,
        reportData.endDate
      );
...
<DefaultLayout title="Margin Report">
  <div class="mb-4 breadcrumbs text-sm">
    <ul>
      ...
      <li class="flex items-center">
        <span class="mr-2">📈</span> Margin
      </li>
    </ul>
  </div>
  <PageHeader title="Laporan Margin" icon="📈" />
  ...
  <div class="p-6">
    ...
    <div class="alert alert-info mb-6">
      <div>
        <span
          >Laporan margin: <strong>{formattedStartDate}</strong> -
    ...
    <!-- Report Summary -->
    <div
      class="card bg-gradient-to-r from-teal-500 to-cyan-600 text-white shadow-xl mb-8"
    >
      <div class="card-body">
        <MarginReportSummary {reportData} />
      </div>
    </div>
    ...
    <div class="card bg-base-100 shadow-xl">
      <div class="card-body">
        <MarginReportDetails {reportData} />
      </div>
    </div>
  </div>
</DefaultLayout>
