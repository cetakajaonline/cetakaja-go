<script lang="ts">
  ...
  import CustomerReportChart from "$lib/components/reports/CustomerReportChart.svelte";
  import CustomerReportSummary from "$lib/components/reports/CustomerReportSummary.svelte";
  import CustomerReportDetails from "$lib/components/reports/CustomerReportDetails.svelte";
  import type { CustomerReportData } from "$lib/types";
  import { onMount } from "svelte";

  export let data: { reportData: CustomerReportData };
  ...
  async function refreshReport(startDate: Date, endDate: Date) {
    ...
    window.location.href = `/reports/customer?startDate=${startFormattedDate}&endDate=${endFormattedDate}`;
  }
  ...
  async function exportToPDF() {
    try {
      const { exportCustomerReportToPDF } = await import('$lib/utils/pdfExporter');
      await exportCustomerReportToPDF(reportData, reportData.startDate, reportData.endDate);
    } catch (error) {
  ...
  async function exportToExcel() {
    try {
      const { exportCustomerReportToExcel } = await import('$lib/utils/excelExporter');
      await exportCustomerReportToExcel(reportData, reportData.startDate, reportData.endDate);
    } catch (error) {
...
<DefaultLayout title="Customer Report">
  <div class="mb-4 breadcrumbs text-sm">
    <ul>
      ...
      <li class="flex items-center">
        <span class="mr-2">👤</span> Customer
      </li>
    </ul>
  </div>
  <PageHeader title="Laporan Pelanggan" icon="👤" />
  ...
  <div class="p-6">
    ...
    <div class="alert alert-info mb-6">
      <div>
        <span>Laporan pelanggan: <strong>{formattedStartDate}</strong> - <strong>{formattedEndDate}</strong></span>
      </div>
    </div>

    <!-- Report Summary -->
    <CustomerReportSummary {reportData} />

    <!-- Chart Section -->
    <div class="mt-8">
      <CustomerReportChart {reportData} />
    </div>

    <!-- Detailed Report -->
    <div class="mt-8">
      <CustomerReportDetails {reportData} />
    </div>
  </div>
</DefaultLayout>