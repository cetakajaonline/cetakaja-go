 ...
 */
export async function exportDailyReportToExcel(reportData: DailyReportData, reportDate: Date): Promise<void> {
  ...
  // 1. Summary Sheet
  const summaryHeaders: (string | number)[] = ['Laporan Harian', new Date(reportData.date).toLocaleDateString('id-ID', {
    day: 'numeric',
    month: 'long',
    year: 'numeric'
  })];
  ...
  const summaryData: (string | number)[][] = [summaryHeaders, emptyRow, ...summaryRows];
  ...
  if (reportData.topSellingProducts.length > 0) {
    const topProductsHeaders: (string | number)[] = ['Nama Produk', 'Jumlah Terjual', 'Total Pendapatan'];
    
    // Helper function to ensure type safety
    const createTopProductRow = (product: typeof reportData.topSellingProducts[number]): (string | number)[] => {
    ...
    const topProductsRows: (string | number)[][] = reportData.topSellingProducts.map(createTopProductRow);
    const topProductsData: (string | number)[][] = [topProductsHeaders, ...topProductsRows];
  ...
  if (reportData.orders.length > 0) {
    const ordersHeaders: (string | number)[] = ['Waktu', 'No. Order', 'Pelanggan', 'Status', 'Total'];
    
    // Helper function to ensure type safety
    const createOrderRow = (order: typeof reportData.orders[number]): (string | number)[] => {
  ...
  if (reportData.expenses.length > 0) {
    const expensesHeaders: (string | number)[] = ['Waktu', 'Kategori', 'Deskripsi', 'Nominal'];
    
    // Helper function to ensure type safety
    const createExpenseRow = (expense: typeof reportData.expenses[number]): (string | number)[] => {
      const dateStr: string = new Date(expense.date).toLocaleDateString('id-ID');
      const categoryStr: string = expense.category.charAt(0).toUpperCase() + expense.category.slice(1);
