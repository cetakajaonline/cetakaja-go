 ...
 */
export async function exportDailyReportToPDF(reportData: DailyReportData, reportDate: Date): Promise<void> {
  try {
    ...
    // Add title
    const title = `Laporan Harian - ${new Date(reportData.date).toLocaleDateString('id-ID', {
      day: 'numeric',
      month: 'long',
      year: 'numeric'
    })}`;
    ...
    doc.text(summaryHeader, doc.internal.pageSize.width / 2, summaryY, { align: 'center' }); // Center text
    doc.setTextColor(0, 0, 0); // Reset text color
    doc.setFont('helvetica', 'normal');

    doc.text(`Total Orders: ${reportData.totalOrders}`, summaryStartX, summaryY + 10);
    doc.text(`Total Pendapatan: ${formatCurrency(reportData.totalRevenue)}`, summaryStartX, summaryY + 20);
    doc.text(`Total Pengeluaran: ${formatCurrency(reportData.totalExpenses)}`, summaryStartX + 95, summaryY + 10);
    doc.text(`Keuntungan: ${formatCurrency(reportData.totalProfit)}`, summaryStartX + 95, summaryY + 20);
    ...
    doc.setTextColor(0, 0, 0);
    doc.text(statusHeader, doc.internal.pageSize.width / 2, statusY, { align: 'center' }); // Center text
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(0, 0, 0); // Reset text color

    // Organize status in a grid layout with colored indicators
    const statusItems = [
      { label: 'Pending', value: reportData.ordersByStatus.pending, color: [255, 165, 0] }, // Orange
      { label: 'Processing', value: reportData.ordersByStatus.processing, color: [65, 105, 225] }, // Royal Blue
      { label: 'Selesai', value: reportData.ordersByStatus.finished, color: [34, 139, 34] }, // Forest Green
      { label: 'Dibatalkan', value: reportData.ordersByStatus.canceled, color: [220, 20, 60] } // Crimson
    ];
    ...
    if (reportData.topSellingProducts.length > 0) {
      ...
      doc.setTextColor(0, 0, 0);
      doc.text(topProductsHeader, doc.internal.pageSize.width / 2, topProductsStartY, { align: 'center' }); // Center text
      doc.setFont(
        'helvetica', 'normal');
      doc.setTextColor(0, 0, 0); // Reset text color

      // Prepare data for the table
      const topProductsData: string[][] = reportData.topSellingProducts.map((product) => {
        const row: string[] = [
          product.name,
    ...
    if (reportData.orders.length > 0) {
      ...
      doc.setTextColor(0, 0, 0);
      doc.text(ordersHeader, doc.internal.pageSize.width / 2, ordersStartY, { align: 'center' }); // Center text
      doc.setFont(
        'helvetica', 'normal');
      doc.setTextColor(0, 0, 0); // Reset text color
    ...
    if (reportData.expenses.length > 0) {
      ...
      doc.setTextColor(0, 0, 0);
      doc.text(expensesHeader, doc.internal.pageSize.width / 2, expensesStartY, { align: 'center' }); // Center text
      doc.setFont(
        'helvetica', 'normal');
      doc.setTextColor(0, 0, 0); // Reset text color
      ...
      autoTable(doc, {
        ...
        columnStyles: {
          ...
          2: { cellWidth: 'auto' }, // Description column - auto adjust
