...
// Define the types for the daily report data
 ...
 */
export async function getDailyReport(targetDate: Date): Promise<DailyReportData> {
  ...
  const endDate = new Date(targetDate.getFullYear(), targetDate.getMonth(), targetDate.getDate() + 1, 0, 0, 0, -1);
  ...
  const totalExpenses = expenses.reduce((sum: number, expense: typeof expenses[number]) => sum + expense.nominal, 0);
  const totalProfit = totalRevenue - totalExpenses;

  // Count orders by status
  const ordersByStatus = {
    pending: orders.filter((order: typeof orders[number]) => order.status === 'pending').length,
    processing: orders.filter((order: typeof orders[number]) => order.status === 'processing').length,
    finished: orders.filter((order: typeof orders[number]) => order.status === 'finished').length,
    canceled: orders.filter((order: typeof orders[number]) => order.status === 'canceled').length
  };

  // Calculate top selling products for the day
  const productSales: Record<number, { name: string; totalSold: number; totalRevenue: number }> = {};
