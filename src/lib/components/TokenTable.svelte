<script lang="ts">
  ...
  import { ChevronUp, ChevronDown, Pencil, Trash2, CheckCircle, XCircle } from 'lucide-svelte';
        ...
        <tr class="hover:bg-base-100 transition">
          ...
          <td>
            <div class="flex items-center gap-3">
              ...
              <div>
                <div class="font-medium text-base-content">{token.creator?.name || '-'}</div>
                <div class="text-xs text-base-content/70">{token.creator?.email || '-'}</div>
              </div>
            </div>
          </td>
          ...
          <td class="text-right space-x-2">
            <IconButton icon={Pencil} color="btn-circle btn-outline btn-success" onClick={() => onEdit(token)} />
            <IconButton icon={Trash2} color="btn-circle btn-outline btn-error" onClick={() => onDelete(token)} />
          </td>
        </tr>
      {/each}
    </tbody>
