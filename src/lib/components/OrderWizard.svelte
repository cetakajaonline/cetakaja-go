<script lang="ts">
  ...
  function addItem() {
    ...
    const existingItemIndex = orderData.orderItems.findIndex(item => 
  ...
  async function handleSubmitOrder() {
    ...
    try {
      ...
      if (customer && editedCustomer.name !== customer.name || 
          editedCustomer.address !== customer.address) {
  ...
  async function handlePaymentProofUpload() {
    ...
    try {
      ...
      const paymentResponse = await fetch(`/api/orders/${orderDetails.id}/payment`);
      ...
      if (paymentResponse.ok) {
        ...
        throw new Error("Gagal mengakses catatan pembayaran. Silakan coba kembali ke halaman pesanan untuk mengunggah bukti pembayaran.");
      }
      ...
      if (!res.ok) {
        ...
        if (res.status === 403) {
          throw new Error("Anda tidak diizinkan mengunggah bukti pembayaran. Silakan coba kembali ke halaman pesanan.");
        }
      ...
      errorMessage = error.message || 'Terjadi kesalahan saat mengunggah bukti pembayaran';
    } finally {
      loading = false;
    }
  }
    ...
    <div class="mb-8">
      <div class="flex items-center justify-between relative">
        {#each [0, 1, 2, 3, 4, 5, 6] as step, i}
          ...
          {#if i < 6}
            <div class={`absolute top-5 left-1/2 w-full h-1 -translate-x-1/2 ${
              currentStep > step ? 'bg-blue-600' : 'bg-gray-300'
            }`} style="width: calc(100% / 6); left: calc({i * 100 / 6}% + 8.333%)"></div>
          {/if}
        {/each}
      </div>
    </div>

    <div class="max-w-4xl mx-auto bg-white rounded-xl shadow-md p-6 border border-gray-300">
      <!-- Step 0: Login/Phone Input -->
      {#if currentStep === 0}
        <div>
          <h2 class="text-2xl font-bold mb-6 text-center text-gray-900">Selamat Datang</h2>
          
          {#if showPasswordForm}
            <!-- Password Form for Existing Customers -->
            <div class="bg-white rounded-xl shadow-md p-6 border border-gray-300">
              <h2 class="text-xl font-semibold mb-4 text-gray-800">Masukkan Password</h2>
              <p class="text-gray-600 mb-4">Akun dengan nomor {phone} ditemukan. Silakan masukkan password Anda.</p>
              
              <div class="form-control w-full">
                <label for="password-input" class="label">
                  <span class="label-text text-gray-700">Password <span class="text-red-600"> *</span></span>
                </label>
                <input
                  id="password-input"
                  type="password"
                ...
                />
                <label for="password-input" class="label">
                  <span class="label-text-alt text-gray-500">Password biasanya adalah nomor telepon Anda</span>
                </label>
              </div>
              
              {#if errorMessage}
                <div class="alert alert-error mt-4 rounded-lg">
                  <svg xmlns="http://www.w3.org/2000/svg" class="stroke-current shrink-0 h-6 w-6 text-red-800" fill="none" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              ...
              {#if successMessage}
                <div class="alert alert-success mt-4 rounded-lg">
                  <svg xmlns="http://www.w3.org/2000/svg" class="stroke-current shrink-0 h-6 w-6 text-green-800" fill="none" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              ...
              <div class="flex gap-3 mt-4">
                <button 
                  ...
                  onclick={() => {showPasswordForm = false; successMessage = ''; errorMessage = ''}}
                  disabled={loading}
                >
                  Kembali
                </button>
          ...
          {:else}
            ...
            <div class="bg-white rounded-xl shadow-md p-6 border border-gray-300">
              <h2 class="text-xl font-semibold mb-4 text-gray-800">Masukkan Nomor Telepon</h2>
              ...
              <div class="form-control w-full">
                <label for="phone-input" class="label">
                  <span class="label-text text-gray-700">Nomor Telepon <span class="text-red-600"> *</span></span>
                </label>
                <input
                  id="phone-input"
                  type="tel"
                ...
                />
                <label for="phone-input" class="label">
                  <span class="label-text-alt text-gray-500">Diawali dengan 08 dan hanya angka</span>
                </label>
              </div>
      ...
      {#if currentStep === 1}
        <div>
          <h2 class="text-2xl font-bold mb-6 text-center text-gray-900">Detail Pelanggan</h2>
          ...
          <div class="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200 space-y-4">
            ...
            <div class="form-control w-full">
              <label for="customerPhone" class="label">
                <span class="label-text text-gray-700">Telepon</span>
              </label>
              <input
                id="customerPhone"
                type="tel"
              ...
              />
              <label for="customerPhone" class="label">
                <span class="label-text-alt text-gray-500">Nomor telepon tidak dapat diubah</span>
              </label>
            </div>
      ...
      {#if currentStep === 2}
        <div>
          <h2 class="text-2xl font-bold mb-6 text-center text-gray-900">Tambah Produk ke Pesanan</h2>
          
          <div class="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
            <div class="form-control w-full mb-4 homepage-search-select">
              <label for="newItemProduct" class="label">
                <span class="label-text text-gray-700">Kaka mau cari produk apa?</span>
              </label>
              <SearchSelect
                id="newItemProduct"
            ...
            {#if newItem.productId}
              <div class="form-control w-full mb-4">
                <label for="newItemVariant" class="label">
                  <span class="label-text text-gray-700">Pilih varian yang diinginkan</span>
                </label>
                <select
                  id="newItemVariant"
              ...
              <div class="form-control w-full mb-4">
                <label for="newItemQty" class="label">
                  <span class="label-text text-gray-700">Kaka mau berapa pcs?</span>
                </label>
                <input
                  id="newItemQty"
                  type="number"
              ...
              <div class="form-control w-full mb-4">
                <label for="newItemNotes" class="label">
                  <span class="label-text text-gray-700">Link File Desain (Google Drive, dll)</span>
                </label>
                <input
                  id="newItemNotes"
                  type="text"
                  ...
                  <table class="table table-zebra">
                    ...
                    <tbody>
                      <tr class="hover:bg-gray-50">
                        <td class="text-gray-800">{products.find(p => p.id === newItem.productId)?.name}</td>
                        <td class="text-gray-800">{newItem.variantId ? (productVariants.find(v => v.id === newItem.variantId)?.variantName || '-') : '-'}</td>
                        <td class="text-gray-800">{newItem.qty}</td>
                        <td class="text-gray-800">{formatCurrency(newItem.price)}</td>
          ...
          {#if orderData.orderItems.length > 0}
            <div class="mb-6">
              <h3 class="text-xl font-semibold mb-3 text-gray-900">Rincian Produk yang Ditambahkan</h3>
                  ...
                  <tbody>
                    {#each orderData.orderItems as item, index}
                      <tr class="hover:bg-gray-50">
                        ...
                        <td class="text-gray-800">{item.variant?.variantName || '-'}</td>
                        <td class="text-gray-800">{item.qty}</td>
                        <td class="text-gray-800">{formatCurrency(item.price)}</td>
                        <td class="text-gray-800">{formatCurrency(item.subtotal)}</td>
                    ...
                    <tr class="font-bold bg-gray-100">
                      <td colspan="5" class="text-right text-gray-900">Total:</td>
                      <td class="text-gray-900">{formatCurrency(calculateTotal())}</td>
                      <td></td>
                    </tr>
                  </tbody>
                </table>
      ...
      {#if currentStep === 3}
        <div>
          <h2 class="text-2xl font-bold mb-6 text-center text-gray-900">Metode Pengiriman & Pembayaran</h2>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <div class="form-control w-full md:col-span-2">
              ...
              <textarea
                ...
                placeholder="Titip ke Si A, atau catatan lainnya"
                rows="3"
              ></textarea>
            </div>
      ...
      {#if currentStep === 4}
        <div>
          <h2 class="text-2xl font-bold mb-6 text-center text-gray-900">Instruksi Pembayaran</h2>
          
          {#if orderData.paymentMethod === 'cash'}
            <!-- Cash Payment Instructions -->
            <div class="mb-6 p-4 bg-yellow-50 rounded-lg border border-yellow-200">
              <h2 class="text-lg font-semibold mb-2 text-gray-900">Instruksi Pembayaran</h2>
              {#if settings?.cashPaymentInstruction}
                <div class="prose text-gray-800">
                  {@html settings.cashPaymentInstruction}
                </div>
              {:else}
                <p class="text-gray-800">Bayar tunai saat barang diterima. Silakan lakukan pembayaran saat kurir mengantarkan pesanan.</p>
              {/if}
            </div>
          {:else if orderData.paymentMethod === 'qris'}
            ...
            <div class="mb-6 p-4 bg-yellow-50 rounded-lg border border-yellow-200">
              ...
              {:else}
                <p class="text-gray-800">Silakan scan QRIS di bawah ini menggunakan aplikasi dompet digital Anda untuk melakukan pembayaran.</p>
              {/if}
            </div>
            
            <div class="mb-6 flex flex-col items-center">
              <h2 class="text-lg font-semibold mb-4 text-gray-900">QRIS Pembayaran</h2>
              {#if settings?.qrisImage}
                ...
                />
              {:else}
                <div class="w-64 h-64 flex items-center justify-center bg-gray-100 border-4 border-gray-300 rounded-lg">
                  <span class="text-gray-500">QRIS belum diatur</span>
                </div>
              {/if}
              <p class="mt-4 text-center font-semibold text-lg text-gray-900">{formatCurrency(getAmountToPay())}</p>
            </div>
          {:else if orderData.paymentMethod === 'transfer'}
            ...
            <div class="mb-6 p-4 bg-yellow-50 rounded-lg border border-yellow-200">
              ...
              {:else}
                <p class="text-gray-800">Silakan lakukan transfer ke rekening berikut sesuai dengan total pesanan:</p>
              {/if}
            </div>
            
            <div class="mb-6 p-6 bg-gray-50 rounded-lg border border-gray-200">
              <h2 class="text-lg font-semibold mb-4 text-center text-gray-900">Detail Rekening</h2>
              
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <p class="mb-2 text-gray-800"><span class="font-medium">Nama Bank:</span></p>
                  <p class="text-xl font-bold bg-white p-2 rounded border border-gray-300 text-gray-900">
                    {settings?.bankName || 'Bank Belum Diatur'}
                  </p>
                </div>
                
                <div>
                  <p class="mb-2 text-gray-800"><span class="font-medium">Kode Bank:</span></p>
                  <p class="text-xl font-bold bg-white p-2 rounded border border-gray-300 text-gray-900">
                ...
                <div class="md:col-span-2">
                  <p class="mb-2 text-gray-800"><span class="font-medium">Nomor Rekening:</span></p>
                  <p class="text-2xl font-bold bg-white p-3 rounded border border-gray-300 text-gray-900">
                    {settings?.bankAccountNumber || 'Rekening Belum Diatur'}
                  </p>
                </div>
                
                <div class="md:col-span-2">
                  <p class="mb-2 text-gray-800"><span class="font-medium">Atas Nama:</span></p>
                  <p class="text-xl font-bold bg-white p-2 rounded border border-gray-300 text-gray-900">
              ...
              <div class="mt-4 p-3 bg-blue-100 rounded border border-blue-200">
                <p class="font-semibold text-gray-900">Total yang harus ditransfer:</p>
                <p class="text-2xl font-bold text-blue-700 text-center">{formatCurrency(getAmountToPay())}</p>
              </div>
            </div>
          {/if}
      ...
      {#if currentStep === 5}
        <div>
          <h2 class="text-2xl font-bold mb-6 text-center text-gray-900">Upload Bukti Pembayaran</h2>
          
          <div class="mb-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
            <p class="text-gray-800 mb-2">ID Pesanan: {orderDetails?.id}</p>
            <p class="text-gray-800 mb-2">Total Pembayaran: {formatCurrency(getAmountToPay())}</p>
            <p class="text-gray-800">Metode Pembayaran: {orderData.paymentMethod === 'cash' ? 'Tunai' : orderData.paymentMethod === 'qris' ? 'QRIS' : 'Transfer Bank'}</p>
          </div>
          
          <div class="mb-6">
            <label class="label font-medium py-2 text-gray-700" for="paymentProofInput">Upload Bukti Pembayaran</label>
            <input
              ...
              <div class="mt-4">
                ...
                {:else}
                  <div class="p-4 bg-gray-100 rounded border border-gray-300">
                    <span class="text-gray-800">File: {paymentProofFile?.name}</span>
                  </div>
                {/if}
              </div>
            {/if}
      ...
      {#if currentStep === 6}
        <div class="text-center py-12">
          ...
          <p class="text-xl text-gray-700 mb-8">Pesanan Anda telah berhasil dibuat dan bukti pembayaran telah diunggah.</p>
          
          <div class="mb-8 p-4 bg-blue-50 rounded-lg border border-blue-200">
            <p class="text-gray-800 mb-2">ID Pesanan: {orderDetails?.id}</p>
            <p class="text-gray-800">Total Pembayaran: {formatCurrency(getAmountToPay())}</p>
          </div>
